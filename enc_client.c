...
* reply then set exit status to 2. Encryption server should only stay connected to
* encryption client.
*/
void validateServer(int socketFD, int port) {
    ...
    // server replies 'terminate' if the first message received is not 'enc_client'
    // Send the message to the server in chunks, loop until the number of characters written
    // to the server equals the number of characters read from the file.
    while (writeCount < 10) {
        charsWritten = write(socketFD, pointer, 10); // write pointer buffer to socket
        if (charsWritten < 0) {
            fprintf(stderr, "CLIENT: Error send() on authorization for port %d", port);
            exit(2); // if enc_client cannot connect to enc_server set exit value to 2
        }
    ...
    // Server sent a terminate message, the port number for the dec_server was used  
...
/*
* enc_client is passed a message filename, key filename and port number for the enc_server.
* main verifies the key is sufficient length, the message contains no invalid characters, 
* and then send both to the encryption server. The encrypted message is printed to stdout
...
int main(int argc, char* argv[]) {
    ...
    // validate the connection to the correct server : enc_client to enc_server
    validateServer(socketFD, atoi(argv[3]));
    // verify the message and key contain correct characters and get their lengths
    ...
    // send the message and the key to the encryption server 
    writeToServer(argv[1], messageLen - 1, socketFD);
    writeToServer(argv[2], keyLen - 1, socketFD);

    // Read the server's encrypted response from the socket and write it to the buffer
    ...
    // print cipher text with newline and close socket
    printf("%s\n", buffer);
    free(buffer);
    close(socketFD);
    exit(0);
